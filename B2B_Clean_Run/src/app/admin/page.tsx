...
import { Lock, AlertCircle, ShoppingBag, FolderKanban, LogOut, Calculator, BarChart3 } from 'lucide-react';
...
export default function AdminPortalPage() {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuthError('관리자 로그인 중 오류가 발생했습니다.');
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 pt-4">
          ...
          >
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <h2 className="text-[17px] font-bold text-neutral-900 tracking-wide">분석 마스터</h2>
                <p className="text-[11.5px] text-neutral-400 font-light leading-relaxed">
                  주문 판매 데이터와 거래처 장바구니 수요를 상품별, 컬러별, 거래처별, 주차별로 취합해 빠른 사입 대응 기준을 확인합니다.
                </p>
              </div>
            </div>
            <div className="pt-8 text-neutral-800 text-[11px] font-semibold tracking-widest uppercase flex items-center gap-1 group-hover:text-[#615b51] transition-colors">
              <span>Go to Analysis Master →</span>
            </div>
          </div>

          {/* Card 4: Wholesale Price Calculator */}
          <div 
