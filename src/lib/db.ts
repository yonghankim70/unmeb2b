...
export function getDbPath(): string {
  const primaryPath = 'Z:\\HDD1\\PDDB';
  const fallbackPath = path.join(process.cwd(), 'pddb_dev');
...
export function backupDbToZDrive(): void {
  try {
    const localDbDir = path.join(process.cwd(), 'pddb_dev');
    const localSqlitePath = path.join(localDbDir, 'pddb.sqlite');
    const zDir = 'Z:\\HDD1\\PDDB';
...
export function getSqliteDb(): Database.Database {
  ...
  // 로컬 파일이 없고 Z 드라이브에 백업이 있으면 최초 복사해 와서 활성화
  if (!fs.existsSync(localSqlitePath)) {
    const zSqlitePath = 'Z:\\HDD1\\PDDB\\pddb.sqlite';
    if (fs.existsSync(zSqlitePath)) {
      try {
        console.log('[DB] Copying Z drive sqlite to local...');
        const dbBuffer = fs.readFileSync(zSqlitePath);
        fs.writeFileSync(localSqlitePath, dbBuffer);
        console.log('[DB] Local database initialized from Z drive.');
      } catch (err) {
        console.error('[DB] Failed to initialize local database from Z drive:', err);
      }
    }
  }
    ...
    db.transaction(() => {
      ...
      const stmt = db.prepare(`
        INSERT INTO products (
          임시코드, 상품명, 업로드일자, 노출여부, 노출제외, 카테고리, 주차, 아이템, 컬러, 사이즈,
...
export function writeCustomers(customers: Customer[]): boolean {
  ...
  try {
    db.transaction(() => {
      ...
      const stmt = db.prepare(`
        INSERT INTO customers (거래처명, 접속코드, 거래처등급, 텔레그램ID, 결제방식, 세금계산서발행, 로그인차단, 최근접속일) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `);
